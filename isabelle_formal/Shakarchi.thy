...
Our comment on the codex statement: totally correct except for an unnessary condition "∀x∈s. f x ≠ 0".
 *)
theorem exercise_1_13c: 
  fixes f::"complex ⇒ complex"
  assumes "open s" "f holomorphic_on s" "∀x y. x ∈ s ⟶ y ∈ s ⟶ norm (f x) = norm (f y)"
  shows "∃ c. ∀x∈s. f x = c"
  oops
...
theorem exercise_1_19a: 
  fixes z::complex
  assumes "norm z = 1"
  shows "∀ε>0. ∃N. ∀n≥N. norm (of_nat n * z^n) ≥ ε" oops
...
theorem exercise_1_19b: 
  fixes z::complex
  assumes "norm z = 1"
  shows "summable (λn. z^n / n^2)" oops
...
theorem not_finite_partition_of_nat_into_arithmetic_progression:
  fixes S::"nat set"
  assumes "finite S" "∀s∈S. ∃a d. s = {a, a + d, a + 2*d, 
...
Our comment on the codex statement: almost correct except for not noticing 
  that vector_derivative of type "real ⇒ 'b::real_normed_vector", which is very understandable.
 *)
theorem exercise_1_26: 
  fixes f::"real ⇒ 'b::real_normed_vector"
  assumes "continuous_on Ω f" 
    "∃g. (∀x∈Ω. (f x) = (vector_derivative g (at x within Ω)))" 
    "∃h. (∀x∈Ω. (f x) = (vector_derivative h (at x within Ω)))"
  shows "∃c. (∀x∈Ω. g x = h x + c)" oops
